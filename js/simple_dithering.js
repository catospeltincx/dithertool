...
//per pixel kijken naar offsets
//=> brightness berekening: huidige brightness + error marge
//berekening gebeurt op lijn 53
const OFFSETS = [
  //1 pixel naar rechts, 0 naar onder, dus pixel ernaast
  [1, 0],
  [2, 0],
  //grijze schijn
  [-1, 1],
...
function jsDither() {
  ...
  for (let x = 0; x < gWidth; x++) {
    for (let y = 0; y < gHeight; y++) {
      ...
      let err;
      //data[pos + 2] = 0xff;
      //om filter te maken: functies onderstaand afzetten en lijn hierboven opzetten
      if (bright <= 127) {
        data[pos + 0] = 0x00;
        data[pos + 1] = 0x00;
        data[pos + 2] = 0x00;
      ...
      for (const [dx, dy] of OFFSETS) {
        ...
        if (x2 < gWidth && y2 < gHeight) {
          //laatste 4: halve negatieve poes met gekleurd randje
